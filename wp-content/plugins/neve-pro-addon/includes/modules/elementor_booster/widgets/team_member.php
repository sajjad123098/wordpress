...
class Team_Member extends Elementor_Booster_Base {
	...
	protected function render() {
		...
		if ( $settings['image']['url'] || $settings['image']['id'] ) {
			...
			echo $image_link; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<figure class="neb-member-figure ' . ( $settings['force_round_avatar'] === 'yes' ? 'force-round' : '' ) . '">';
			echo Group_Control_Image_Size::get_attachment_image_html( $settings, 'thumbnail', 'image' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '</figure>';
			echo $closing_link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}

		echo '<div class="neb-member-body">';

		if ( $settings['title'] ) {
			echo $header_link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			printf(
				'<%1$s %2$s>%3$s</%1$s>',
